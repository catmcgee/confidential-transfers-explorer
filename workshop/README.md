...
| 03 | **Send** — first to an unconfigured recipient (the app's "recipient has not configured" error), then for real with the multi-transaction progress bar | [`03-confidential-transfer.ts`](03-confidential-transfer.ts) | [`03-confidential-transfer.md`](03-confidential-transfer.md) | Why sending to an unconfigured account fails; the 3 ZK proofs, the 1232-byte limit, context-state scratch accounts, ~5 transactions |
| 04 | **Configure Confidential** + two signature prompts | [`04-configure-account.ts`](04-configure-account.ts) | [`04-configure-account.md`](04-configure-account.md) | Keys derived from signatures over readable text; ElGamal pubkey published on-chain — the fix for step 03's failure, proven by re-deriving and matching the on-chain pubkeys |
